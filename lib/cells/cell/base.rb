...
module Cells
  module Cell
    ...
    # extra options argument hash, just like you would pass locals in partials.
    ...
    class Base
      ...
      IGNORED_INSTANCE_VARIABLES = ['@controller'].freeze
      ...
      class << self
        ...
        def find_class_view_for_state(state)
          if superclass == ::Cell::Base
            [view_for_state(state)]
          else
            self.superclass.find_class_view_for_state(state) << self.view_for_state(state)
          end
        end
        ...
        # == Example:
        #
        #   UserCell.cell_name   # => "user"
        #
        def cell_name
          self.name.underscore.sub(/_cell/, '')
        end
        ...
        # == Example:
        #
        #   ::Cell::Base.class_from_cell_name(:user)  # => UserCell
        #
      ...
      def render_view_for_backward_compat(options, state)
        ...
        self.render_view_for(options, state)
      end
      ...
      # part of the page. Just use <tt>:view</tt> and enjoy.
      #
      def render(options = {})
        self.render_view_for(options, @state_name)  ### FIXME: i don't like the magic access to @state_name here. ugly!
      end
      ...
      def render_view_for(options, state)
        ...
        if options[:text] || options[:inline] || options[:file]
          # do nothing
        elsif options[:state]
          options[:text] = self.render_state(options[:state])
        else
          # handle :layout, :template_format, :view
          options = self.defaultize_render_options_for(options, state)

          # set instance vars, include helpers:
          self.prepare_action_view_for(action_view, options)

          template = self.find_family_view_for_state_with_caching(options[:view], action_view)
          options[:file] = template
        end

        action_view.render_for(self.sanitize_render_options(options))
      end

      # Defaultize the passed options from #render.
      def defaultize_render_options_for(options, state)
        options.reverse_merge(:template_format => self.class.default_template_format, :view => state)
      end

      def prepare_action_view_for(action_view, options)
        self.include_helpers_in_class(action_view.class)
        action_view.assigns = assigns_for_view # make instance vars available.
        action_view.template_format = options[:template_format]
      end
      ...
      # Prepares <tt>options</tt> to be passed to ActionView::Base#render by removing
      # unknown parameters.
      def sanitize_render_options(options)
        options.except(:view, :state)
      end
      ...
      def find_family_view_for_state(state, action_view)
        ...
        self.possible_paths_for_state(state).each do |template_path|
      ...
      def find_family_view_for_state_with_caching(state, action_view)
        return self.find_family_view_for_state(state, action_view) unless self.class.cache_configured?
        key = "#{state}/#{action_view.template_format}"
        self.class.state2view_cache[key] ||= self.find_family_view_for_state(state, action_view)
      end
      ...
      def assigns_for_view
        assigns = {}
        (self.instance_variables - IGNORED_INSTANCE_VARIABLES).each do |v|
         assigns[v[1..-1]] = self.instance_variable_get(v)
        end
        assigns
      end
      ...
      def log(message)
        @controller.logger.debug(message) if @controller.logger.present?
      end
